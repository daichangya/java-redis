...
import com.daicy.redis.client.utils.RedisCodecUtil;
 ...
 */
@UnstableApi
public final class SyncRedisDecoder extends ByteToMessageDecoder {
     ...
     */
    public SyncRedisDecoder(boolean decodeInlineCommands) {
     ...
     */
    public SyncRedisDecoder(int maxInlineMessageLength, RedisMessagePool messagePool) {
     ...
     * @param decodeInlineCommands if {@code true}, inline commands will be decoded.
     */
    public SyncRedisDecoder(int maxInlineMessageLength, RedisMessagePool messagePool, boolean decodeInlineCommands) {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        try {
            ...
            if(CollectionUtils.isNotEmpty(out) && !(out.get(0) instanceof SimpleStringRedisMessage)){
                if (null != ctx.pipeline().get("syncRedisDecoder")) {
                    ctx.pipeline().replace("syncRedisDecoder", "redisDecoder", new RedisDecoder());
                }
            }
        }
    }
    ...
    private boolean decodeBulkStringContent(ByteBuf in, List<Object> out) throws Exception {
        ...
        if (readableBytes >= remainingBulkLength ) {
