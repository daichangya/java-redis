...
@ParamLength(1)
@PubSubAllowed
public class PatternSubscribeCommand extends SubscriptionSupport {
    ...
    public RedisMessage pubsubSubscribeChannel(Request request, String channel) {
        ...
        Set<String> pubsubPatterns = clientSession.getPubsubPatterns();
        if (pubsubPatterns.add(channel)) {
            // 关联示意图
            // {
            //  频道名        订阅频道的客户端
            //  'channel-a' : [c1, c2, c3],
            ...
            HashMap<String, List<String>> serverPubsubPatterns = redisServerContext.getPubsubPatterns();
            List<String> clientSessions = serverPubsubPatterns
                    .getOrDefault(channel, Lists.newArrayList());
            clientSessions.add(clientSession.getId());
            serverPubsubPatterns.put(channel, clientSessions);
            // before:
            // 'channel' : [c1, c2]
            // after:
            // 'channel' : [c1, c2, c3]
