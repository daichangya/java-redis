...
import com.daicy.collections.CowOrderedMap;
 ...
 */
public final class CowTreeMap<K, V> extends AbstractMap<K, V> implements CowOrderedMap<K, V>,SortedMap<K,V> {
    ...
    private static class NodeStack<K, V> {
        ...
        static <K, V> NodeStack<K, V> before(K upperBound, Node<K, V> root, Comparator<K> cmp, boolean toInclusive) {
            if (root.numKeys == 0)
                return null;
            ...
            for (; ; ) {
                ...
                if (index >= 0) {
                    NodeStack<K, V> nodeStack = new NodeStack<>(cur, node,
                            toInclusive ? Math.min(insertion, node.numKeys - 1) : insertion);
                    return toInclusive ? nodeStack : nodeStack.previous();
                } else if (node.isLeaf()) {
                    NodeStack<K, V> nodeStack = new NodeStack<>(cur, node,
                            toInclusive ? Math.min(insertion, node.numKeys - 1) : insertion);
                    return toInclusive ? nodeStack : nodeStack.previous();
        ...
        static <K, V> NodeStack<K, V> after(K lowerBound, Node<K, V> root, Comparator<K> cmp, boolean fromInclusive) {
            if (root.numKeys == 0)
                return null;
            ...
            for (; ; ) {
                ...
                if (index >= 0) {
                    NodeStack<K, V> nodeStack = new NodeStack<>(cur, node,
                            fromInclusive ? Math.max(index, 0) : index);
                    return fromInclusive ? nodeStack : nodeStack.next();
                } else if (node.isLeaf()) {
                    NodeStack<K, V> nodeStack = new NodeStack<>(cur, node,
                            fromInclusive ? Math.max(insertion - 1, 0) : insertion - 1);
                    return fromInclusive ? nodeStack : nodeStack.next();
                } else {
    ...
    private class AscendingEntryIter extends EntryIter {
        ...
        AscendingEntryIter(Node<K, V> root, K lowerBoundExclusive) {
            this(root,lowerBoundExclusive,false);
        }

        AscendingEntryIter(Node<K, V> root, K lowerBoundExclusive, boolean fromInclusive) {
            stack = NodeStack.after(lowerBoundExclusive, root, comparator,fromInclusive);
        }
    ...
    private class DescendingEntryIter extends EntryIter {
        ...
        DescendingEntryIter(Node<K, V> root, K upperBoundExclusive) {
            this(root,upperBoundExclusive,false);
        }

        DescendingEntryIter(Node<K, V> root, K upperBoundExclusive, boolean toInclusive) {
            stack = NodeStack.before(upperBoundExclusive, root, comparator, toInclusive);
        }
    ...
    private class AscendingSubEntryIter extends AscendingEntryIter {

        private K upperBoundExclusive;

        private boolean toInclusive;

        AscendingSubEntryIter(Node<K, V> root, K lowerBoundExclusive, boolean fromInclusive,K upperBoundExclusive, boolean toInclusive) {
            super(root,lowerBoundExclusive,fromInclusive);
        ...
        public boolean hasNext() {
            if (toInclusive) {
                return stack != null && comparator.compare(stack.getKey(), upperBoundExclusive) <= 0;
            } else {
                return stack != null && comparator.compare(stack.getKey(), upperBoundExclusive) < 0;
            }
        }
    ...
    @Override
    public Iterable<Entry<K, V>> descendingEntriesBefore(K upperBoundExclusive, boolean toInclusive) {
        return () -> new DescendingEntryIter(root, upperBoundExclusive, toInclusive);
    }

    @Override
    public Iterable<Entry<K, V>> ascendingSubEntries(K lowerBoundExclusive, boolean fromInclusive, K upperBoundExclusive, boolean toInclusive) {
        return () -> new AscendingSubEntryIter(root, lowerBoundExclusive,fromInclusive
                ,upperBoundExclusive,toInclusive);
    }

    @Override
    public Iterable<Entry<K, V>> ascendingEntriesAfter(K lowerBoundExclusive,boolean fromInclusive) {
        return () -> new AscendingEntryIter(root, lowerBoundExclusive,fromInclusive);
    }
